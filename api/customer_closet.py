...
def lookup_customer_closet(payload: LookupRequest):
    try:
        ...
        found = closet_data["total_pedidos"] > 0 or len(closet_data["closet_products"]) > 0
        ...
        if not found:
            return {
                ...
                "debug": {
                    ...
                    "closet_final_count": len(closet_data["closet_products"]),
                    "messages": ["Nenhum pedido encontrado para este e-mail."],
                },
            }
        ...
        return {
            ...
            "closet_products": closet_data["closet_products"],
            "debug": {
                "email_input": payload.email,
                "email_normalized": email,
                "cliente_found": cliente_encontrado is not None,
                ...
                "closet_final_count": len(closet_data["closet_products"]),
                "messages": [
                    f"{closet_data['total_pedidos']} pedido(s) encontrado(s)",
                    f"{len(closet_data['closet_products'])} peça(s) no closet",
                ],
            },
        }
